...
3. **Use the host adapter.** Discover session history through the current host's supported commands or storage. For Codex, read [`references/hosts/codex.md`](references/hosts/codex.md). For Pi, read [`references/hosts/pi.md`](references/hosts/pi.md). For another host, use its documented session mechanism. If none exists, say that session history is unavailable and continue from repository state and user-provided context.
