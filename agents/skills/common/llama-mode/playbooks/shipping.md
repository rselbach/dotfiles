...
1. **Verify every PR independently before arming anything.** Use one independent verifier per PR, not a batched review. Each verifier exercises the real surface through an available control skill against parent versus head and returns `PASS`, `PASS+NOTES`, or `FAIL`. Post the verdict only when the task authorizes PR comments. Safe means a verdict from an agent that did not write the code. CI green and bot approval are inputs, not verdicts.
