...
Use this reference when the Babysit playbook handles automated-review comments. The goal is not to ignore bots by default. The goal is to stop treating every automated comment as a required code change.
...
- Skip when: The PR description, screenshots, design review, or nearby code makes the visual change explicit, and the automated comment only restates that a shared visual default changed.
...
- Confidence: candidate
- Skip when: An automated reviewer flags an export, component, helper, or file as unused, and the stacking tool, upper-stack diffs, or PR context proves a later PR uses it.
...
- Skip when: The comment body or a later automated reply says the finding is withdrawn, compliant, or a false positive, and the agent can verify the relevant rule locally.
...
- Confidence: candidate
- Skip when: Practically never. When a diff replaces native browser behavior with a manual equivalent, automated logic findings against that code deserve direct verification.
...
- Source: one sticky-occlusion PR with six automated-review passes; every finding was fixed rather than dismissed.
...
- Skip when: Never skip the verification itself; it costs one command. When a PR
  ...
  a SKILL.md, snapshot of doc wording), and a reviewer claims "the test no longer
