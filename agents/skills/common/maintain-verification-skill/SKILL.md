...
description: "Periodic pass that keeps a project's verification skill and feature map honest: parallel source readers per feature, one live session driving every feature, at most one PR of proven corrections. Use for the `maintain-verification-skill` skill or \"audit the verify skill\"."
...
0. **Locate the target.** Resolve the project's skill root from repository conventions or the current harness, then find the verification skill whose body has launch/drive sections and a feature map (usually `verify-*/`). Several candidates → ask which one; none → stop and point at the `create-verification-skill` skill instead of inventing a target.
