...
description: "Keep a reviewable decision trail for long-running or unattended work: a TSV log with one row per decision (what, why, evidence, result). Local by default; commit it when a reviewer needs the trail to trust the result. Use for the `show-me-your-work` skill, autonomous or multi-phase runs, or work a human reviews after stepping away."
...
At the end of the run, before handing back, check the log told the truth. Locate this run's transcript through the current harness and use only the active workspace's transcript directory. Never search transcript roots for unrelated workspaces. If no transcript is available, audit against the session digest and durable artifacts. Walk the log against what actually happened:
