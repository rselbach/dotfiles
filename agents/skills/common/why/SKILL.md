...
to cover multiple MCPs.

Use the configured `why investigators` role when the host exposes model selection. Give investigators the narrowest permissions that still allow their evidence source. They must not write or post. Some hosts remove connector access from a read-only sandbox; when that applies, use a tool-enabled session with an explicit no-write brief.
