...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, the active selection or caret, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list MCP servers through the current harness or MCP gateway. If discovery is unavailable, inspect only the tools already exposed to this session.
...
For each investigator, use the configured why-investigators model when model selection is available. Choose a delegation mode that preserves MCP access and forbid writes in the prompt. Do not use a read-only mode if the harness removes MCP tools from that mode.
...
Spawn one synthesizer subagent with the configured why-synthesizer model when model selection is available. Its quality check spot-verifies citations, so choose a mode that preserves MCP access and forbid writes in the prompt.
