...
description: "Layered technical-writing standard using Diátaxis structure, Google developer style, STE instruction rules, and Global English syntax. Use when explicitly invoked or when writing or reviewing docs, RFCs, readmes, PR descriptions, or commit messages."
---
...
Don't invent jargon. Use the words a developer would say out loud: "move", "delete", "a budget that only decreases", not "evacuate", "ratchet", or "endgame". A named pattern is fine when the doc says what it means the first time. Suggest additions to `unslop` separately unless updating that skill is part of the authorized task.
...
- Format code snippets using the language and repository conventions. Write real paths and real symbols. Make every count or tree claim true at the commit that lands it, and include the command that regenerates it.
