...
description: "Generate a project-local verification skill that drives your app the way a user does — any language, framework, or platform. Use for the `create-verification-skill` skill, \"make a control skill for this repo\", or when a project has no scripted way to prove UI/CLI/service behavior."
disable-model-invocation: true
---

# Create a verification skill

Every serious project needs a scripted way to drive the real app and prove behavior: launch it, exercise a feature the way a user would, and capture evidence. This skill generates a project-local `verify-<app>` skill tailored to the repo. Resolve the project's skill root from repository conventions or the current harness before writing. You write the generator's output for the next agent, not for a human: it will be read cold, mid-task, by an agent that has never seen the app.
...
Write `<project-skill-root>/verify-<app>/SKILL.md` with YAML frontmatter (`name: verify-<app>` and a `description` that names the app, the surface, and when to reach for it) and these sections, each grounded in what the interview actually found (no placeholders left). Follow the active harness's registration requirements rather than assuming frontmatter alone registers the skill.
