...
1. Delegate to the registered `Comment Sicko` reviewer profile and pass the scope without restating its rules. If that profile is unavailable, launch a normal read-only reviewer with this skill's scope and the profile's repository prompt.
