...
it("annotates automated review threads with distinct pass counts", () => {
  const response = {
    data: {
      repository: {
        pullRequest: {
  ...
  expect(threads.map((thread) => thread.automatedReviewPasses)).toEqual([
    3,
    3,
  ]);
});
