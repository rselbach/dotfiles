...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through the harness's delegation tool. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out, to conserve context. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, and spawns its own workers and verifiers when the delegation backend supports nesting. Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Prefer an isolated remote worker when the harness provides one. Use a local worker when the task needs a local control harness, workspace transcripts, simulators, IDE state, or credentials available only on this machine. Workers that cannot read the local store get required context in their briefs or through repository paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Workers with access to the local store may reference the standing-orders file by path. Paste it verbatim for remote workers and every resume.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the remote-worker default and local exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Run large restacks on a remote worker when available; a local restack at this scale can monopolize the user's machine.
...
- Never resume an agent to check on it; some backends restart an idle agent on resume. Probe read-only through the delegation backend, the ledger, `units.tsv`, `gh`, and pushed branches. Transcript mtime is not liveness.
...
- After an agent harness or session restart, do not assume which workers survived. Re-read the standing orders and `units.tsv`, recompute the frontier, reconcile workers through the delegation backend and durable PR or branch state, respawn one sub-coordinator per track from its stored brief plus current state when needed, drain, and resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
