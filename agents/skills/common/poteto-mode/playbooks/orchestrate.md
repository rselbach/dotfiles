...
Open a task-specific plan preserving the applicable coordination requirements. Omit irrelevant steps instead of copying this playbook verbatim.

#### Roles and placement

- **Coordinator (this chat).** Frames work, authors briefs, drains the inbox, owns the human report, and makes judgment calls. It does not author code. Conflicted merges, restacks, and code changes are worker tasks. It may mechanically land a verified unit only when the user's task and repository policy already authorize the required branch and push operations. Spawn, steer, wait for, and stop agents through the current host's delegation controls. State reads and writes go through `scripts/orch/orch.ts`; the CLI itself never spawns or wakes agents.
- **Sub-coordinator.** Durable, one per track, and only when the program exceeds what one coordinator can drain. A track the coordinator can drain itself needs no middle layer. It owns its units and boards, authors briefs, spawns workers and verifiers when the host permits nested delegation, and rolls up aggregates instead of forwarding raw reports. Cap in-flight children at what one drain can process.
- **Worker / verifier.** Choose local or remote execution from actual dependencies. Local application state, simulators, credentials, and host-only tools require local execution. Remote agents need accessible branches and artifacts because they cannot see local stores. Prefer fewer, broader workers and one writer per worktree or branch. Use an independent verifier when the risk warrants it.
...
Create `.audit/orchestrate/<project-slug>/` in the working repository unless the host provides a task-local durable store. Keep it uncommitted unless the user wants the record reviewed in version control. Every file has exactly one writer; owners publish facts and readers aggregate at read time. Use `bun scripts/orch/orch.ts` for bookkeeping, written below as `orch`, while its TSV and JSON stay readable without the CLI.

- `preferences.md` is the standing-orders register: numbered lines, one constraint each. Include it in every fresh or resumed brief because directives decay across sessions. When you catch yourself restating an instruction, append it before acting.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates with the question, options, and default so completion traffic cannot hide them.
...
Size the brief to the unit. A one-command unit gets a paragraph that still names goal, scope, verification, and report shape. Workers that can read the durable store may receive a path; remote workers and resumed sessions receive the relevant standing orders directly.
...
- Never resume or wake an agent merely to check on it. Probe read-only state such as the ledger, `units.tsv`, PRs, pushed branches, and the host's agent-status view. Session-file modification time is not liveness.
...
- After a host restart, assume local agents stopped unless the host proves otherwise. Re-read standing orders and `units.tsv`, recompute the frontier, reattach remote work by durable artifacts such as PR and branch rather than ephemeral agent ID, then respawn only the coordinators still needed.

#### Escalation

Reaches the human, batched into the status page rather than per item: actions that require new authority, destructive or irreversible operations, shared-history changes, deploys, deletions, external communication, genuine product choices, contradictory standing orders, and program-level dead ends. Park each as a `gates.md` entry before asking and route other work around it.

Ordinary in-scope diagnostics, bounded retries, CI flake triage, review analysis, and format fixes do not need a new question when the task and repository policy already authorize them. When authorization is unclear, do not infer it from this playbook.
