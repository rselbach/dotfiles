...
3. Plan the fix from the trace. Use `architect` when the fix introduces or substantially reshapes an architectural boundary with multiple viable designs. Delegate implementation with the configured `perf-issue` role when supported and review the diff; otherwise implement directly. Capture a post-fix trace.
