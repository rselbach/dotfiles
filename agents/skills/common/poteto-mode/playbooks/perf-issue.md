...
3. Plan the fix from the trace. If it crosses a function boundary, use `architect` first. Delegate implementation with the configured `perf-issue` role when supported and review the diff; otherwise implement directly. Capture a post-fix trace.
