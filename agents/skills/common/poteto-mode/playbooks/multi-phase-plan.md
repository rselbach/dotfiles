...
3. Explore in subagents using the registered `poteto-agent` profile when available and select a model per the Subagents section (the **guard-the-context-window** principle skill). Otherwise include the relevant poteto-mode constraints in each brief. Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
**Control skill.** Pick it by surface. Use a registered control skill or the repository's own harness for browser, Electron, web, CLI, TUI, and native-mobile work. A PR that touches two surfaces gets lanes on both. A surface with no control skill or harness is a risk in Appendix C, and its live block still names how each lane drives it.
...
- [ ] On her go, create a durable objective record with this exact text using the harness's goal mechanism or the program store. "<The plan path, the PR ids in order, the verification rule, who merges, and the done condition.>"
...
- [ ] Arm the 30-minute audit tick with the harness's persistent scheduler or an external watcher with a durable checkpoint. Never leave the cadence to memory.
- [ ] Use this tick prompt, verbatim. "Re-read the execution playbook from trunk and the durable objective record. Audit the operation against both and fix drift in this tick. Probe every active lane and judge progress by side effects only. Stand down a stuck lane and dispatch its replacement now. Then send the operator a status message, whether or not anything changed, with the queue table of PR, owner, state, and head SHA, the verdicts since the last tick, what merged, open operator gates, and blockers."
...
- [ ] Run the available diff-cleanup workflow before each commit and the `no-comments` skill before review.
...
Each live lane runs in its own isolated worker environment at the PR head. Drive through a registered control skill or the repository's own harness.
