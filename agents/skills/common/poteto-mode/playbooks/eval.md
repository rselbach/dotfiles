...
6. **Verify the chain from transcripts, not self-report.** Locate each candidate's transcript through the current harness and use only the active workspace's transcript directory. Never search transcript roots for unrelated workspaces. If transcripts are unavailable, grade from available execution logs and artifacts and state the evidence gap. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
