...
6. **Verify the chain from evidence, not self-report.** Use the host's run record when available, plus tool receipts, changed files, decision logs, and verification output. If session history is needed, invoke **recall** with an exact candidate-session scope. Check which skill files and source files the candidate actually read. Citing a principle is not reading its leaf skill, and reading it is not applying it.
