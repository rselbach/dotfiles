...
4. Verify each component against its baseline via image diff on the matching surface through the available control skill. A nonzero diff is a fail; investigate the pixel delta instead of waving it through. Iterate per component until the diff is zero.
