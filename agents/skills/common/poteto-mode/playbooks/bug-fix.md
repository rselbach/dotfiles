...
3. Plan the fix. If it crosses a function boundary, use `architect` first. Delegate implementation with the configured `bug-fix` role when the host supports it, give the worker a specific scope, and review the diff. Otherwise implement directly from the same scoped brief.
