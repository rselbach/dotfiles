...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a delegated-worker URL handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. Use a transcript exposed by the current harness for the active workspace, a delegated-worker URL, a handoff note, or a pushed branch. Never search transcript roots for unrelated workspaces. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
