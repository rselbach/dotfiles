...
Spawn all explorers concurrently when supported. Use the configured `how explorer` role when the host exposes model selection. Keep each explorer read-only. If subagents are unavailable, explore the angles sequentially and keep a separate finding block for each.

Each explorer gets the same base prompt from `references/explorer-prompt.md` plus a specific exploration angle naming its slice. Each explorer should:
- Start broad with path search for relevant directories and content search for key types, interfaces, and class names
...
Once all explorers return, spawn one read-only subagent to synthesize their findings into a coherent explanation, using the configured `how explainer` role when available. If subagents are unavailable, synthesize directly.
...
After the explanation is complete, spawn one architectural critic per entry in the configured `how critics` role. Without a configured panel, use two or three independent critics and let the host choose their models. Run them concurrently when supported.

Keep every critic read-only. The lead may request stronger reasoning when the host supports it and the architecture warrants it.
