...
_zellij_write_dev_layout() {
  ...
  if [[ -n "${ai2}" ]]; then
    printf \
      ...
      >> "${layout_path}" || return 1
    _zellij_write_command_pane \
      "${layout_path}" '        ' "${ai}" "${ai}; exec bash" false ||
      return 1
    _zellij_write_command_pane \
      "${layout_path}" '        ' "${ai2}" "${ai2}; exec bash" false ||
      return 1
    printf '      }\n' >> "${layout_path}" || return 1
  else
    _zellij_write_command_pane \
  ...
  printf '    }\n' >> "${layout_path}" || return 1
  _zellij_write_command_pane \
    "${layout_path}" '    ' 'shell' 'exec bash' false || return 1
...
_zellij_write_swarm_tree() {
  ...
  if (( count == 1 )); then
    _zellij_write_command_pane \
      "${layout_path}" "${indent}" '' "${pane_command}; exec bash" \
      "${focus_first}"
    return
  fi
