...
tdlm() {
  ...
  for dir in "${base_dir}"/*/; do
    [[ -d "${dir}" ]] || continue
    dirpath="${dir%/}"
    printf -v dirpath_quoted '%q' "${dirpath}"
    printf -v ai_quoted '%q' "${ai}"
    tdl_command="tdl ${ai_quoted}"
    if [[ -n "${ai2}" ]]; then
      printf -v ai2_quoted '%q' "${ai2}"
...
tsl() {
  ...
  tmux rename-window \
    -t "${TMUX_PANE}" "$(basename "${current_dir}")" || return 1
  ...
  while (( ${#panes[@]} < count )); do
    ...
    panes+=("${new_pane}")
    tmux select-layout -t "${panes[0]}" tiled || return 1
  done
  ...
  tmux select-pane -t "${panes[0]}" || return 1
}
